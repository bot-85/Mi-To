...
def masuk():
    ...
    if rom in(""):
    	...
    	try:
            ...
            print ('\n%s[✓] Login Berhasil, Subscribe Channel Gw Dulu Ya '%(H));jeda(5)
        ...
        if nanya in(""):
        	print ("%s[!] saya bertanya wajib di jawab"%(P));jeda(2);masuk()
        elif nanya in("y","Y"):
    	...
    	exit('[•] Selamat Tinggal Sayang Hehe\n')
    else:
    	print("%s[•] Isi yang benar kentod "%(M));exit()
                ...
                if pwx == '':
                    ...
                    def zona(zafi_=None): 
                        ...
                        if ind == '':
                            ...
                            print '%s[%s•%s] akun %sCP %ssaved in > %shasil/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.3)
...
def hasill(c):
	if c in[""]:
		...
		try:
			...
			print("%s[%s•%s] Hasil Crack Akun Fb"%(P,K,P));jeda(2)
