...
# DUMP PUBLIK HEHE
def publik(nisa,headers=header):
    try:
        os.mkdir('dump')
    except:pass
    try:
        ...
        exit('%s╚══[•] dump selesai tod...'%(P))
# DUMP FOLLOWERS HEHE
def followers(nisa,headers=header):
    try:
        os.mkdir('dump')
    except:pass
    try:
        ...
        exit('%s╚══[•] dump selesai tod...'%(P))
# DUMP POSTINGAN HEHE
def postingan(nisa,headers=header):
    try:
        os.mkdir('dump')
    except:pass
    try:
    ...
    def romiy(self):
        ...
        if unikers in ('Y', 'y'):
            ...
            while True:
                ...
                if pwx == '':
                    print '%s╚══[!] jangan kosong kentod ] '%(M)
                elif len(pwx)<=5:
	...
	if c in[""]:
		...
		if rom in['1','01']:
			try:
				...
				exit (M+"╚══[•] tidak ada hasil awokawok ]")
		elif rom in['2','02']:
			try:
				...
				exit (M+"╚══[•] tidak ada hasil awokawokawok ]")
		else:
			exit()
def igg():
