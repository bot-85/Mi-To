...
def menu():
    ...
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sCrack From Publik %s'%(O,P,O,P,jid))
    print('%s╠══[%s2%s] %sCrack From Followers %s'%(O,P,O,P,jid))
    print('%s╠══[%s3%s] %sCrack From like postingan %s'%(O,P,O,P,jid))
...
def var_author():
    ...
    mlaku('%s║     • %sMoch Yayan Juanda Alvero-XD'%(O,P))
    mlaku('%s║'%(O))
def var_ugen():
